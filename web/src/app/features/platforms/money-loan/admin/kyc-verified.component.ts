    ...
    <div class="p-4 md:p-6">
      ...
      <div class="mb-4">
        <div class="flex items-center gap-3 mb-1">
          ...
          <h1 class="text-xl font-bold text-gray-900 dark:text-white">KYC Verified Customers</h1>
        </div>
        <p class="text-xs text-gray-600 dark:text-gray-400 ml-11">All verified and approved customers</p>
      </div>

      <!-- Compact Stats Row -->
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-3 border border-green-200 dark:border-green-800">
          <p class="text-xs text-green-600 dark:text-green-400 mb-0.5">Total Verified</p>
          <p class="text-xl font-bold text-green-700 dark:text-green-300">{{ totalVerified() }}</p>
        </div>
        <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 border border-blue-200 dark:border-blue-800">
          <p class="text-xs text-blue-600 dark:text-blue-400 mb-0.5">This Month</p>
          <p class="text-xl font-bold text-blue-700 dark:text-blue-300">{{ verifiedThisMonth() }}</p>
        </div>
        <div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3 border border-purple-200 dark:border-purple-800">
          <p class="text-xs text-purple-600 dark:text-purple-400 mb-0.5">This Week</p>
          <p class="text-xl font-bold text-purple-700 dark:text-purple-300">{{ verifiedThisWeek() }}</p>
        </div>
        <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-3 border border-indigo-200 dark:border-indigo-800">
          <p class="text-xs text-indigo-600 dark:text-indigo-400 mb-0.5">Avg Score</p>
          <p class="text-xl font-bold text-indigo-700 dark:text-indigo-300">{{ averageScore() }}</p>
        </div>
      </div>
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <div class="flex-1">
            <input
              ...
              class="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent">
          </div>
          <select
            [(ngModel)]="sortBy"
            (ngModelChange)="onSort()"
            class="px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500">
          ...
          <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
              <tr>
                ...
                <th class="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase hidden lg:table-cell">ID</th>
                <th class="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase">Score</th>
                <th class="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase hidden sm:table-cell">Verified</th>
                  ...
                  <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td class="px-3 py-2">
                      <div class="flex items-center gap-2">
                        <div class="w-8 h-8 bg-gradient-to-br from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">
                    ...
                    <td class="px-3 py-2 hidden lg:table-cell">
                      <div class="text-xs">
                        ...
                        <p class="text-gray-500 dark:text-gray-400 font-mono">{{ customer.idNumber || 'N/A' }}</p>
                      </div>
                    </td>
                    <td class="px-3 py-2">
                    ...
                    <td class="px-3 py-2 hidden sm:table-cell">
                      <div class="text-xs">
                        ...
                        @if (customer.verifiedBy) {
                          <p class="text-gray-500 dark:text-gray-400">by {{ customer.verifiedBy }}</p>
                        }
                      </div>
                    </td>
                    <td class="px-3 py-2">
                      <div class="flex items-center justify-end gap-1">
                        ...
                        <button
                          (click)="revokeVerification(customer)"
...
export class KycVerifiedComponent {
  private http = inject(HttpClient);
  private toastService = inject(ToastService);

  customers = signal<VerifiedCustomer[]>([]);
  loading = signal(true);
  searchQuery = '';
  sortBy = 'recent';
  ...
  totalVerified = computed(() => this.customers().length);
  
  verifiedThisMonth = computed(() => {
    const now = new Date();
  ...
  verifiedThisWeek = computed(() => {
    const now = new Date();
    const firstDayOfWeek = new Date(now.setDate(now.getDate() - now.getDay()));
    return this.customers().filter(c => 
      new Date(c.updatedAt) >= firstDayOfWeek
    ).length;
  });
  ...
  loadCustomers() {
    ...
    this.http.get<any>('http://localhost:3000/api/money-loan/customers', {
      ...
      error: (error) => {
        console.error('Error loading verified customers:', error);
        this.toastService.error('Failed to load verified customers');
        this.loading.set(false);
      }
    });
  }
  ...
  viewDetails(customer: VerifiedCustomer) {
    console.log('View customer:', customer);
    this.toastService.info('Customer details - Coming soon');
  }

  revokeVerification(customer: VerifiedCustomer) {
    if (confirm(`Revoke KYC verification for ${customer.fullName}?`)) {
      this.http.put<any>(`http://localhost:3000/api/money-loan/customers/${customer.id}`, {
        kycStatus: 'pending'
      }).subscribe({
        next: (response) => {
          if (response.success) {
            this.toastService.success('Verification revoked successfully');
            this.loadCustomers();
          }
        },
        error: (error) => {
          console.error('Error revoking verification:', error);
          this.toastService.error('Failed to revoke verification');
        }
      });
    }
  }
