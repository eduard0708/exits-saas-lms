-- Grant all cash management permissions to all cashier roles

DO $$
DECLARE
    v_permission_id INT;
    v_role_record RECORD;
    v_perm_key TEXT;
BEGIN
    -- Loop through all cash permissions
    FOR v_perm_key IN 
        SELECT permission_key 
        FROM permissions 
        WHERE permission_key LIKE 'money-loan:cash:%'
    LOOP
        SELECT id INTO v_permission_id
        FROM permissions
        WHERE permission_key = v_perm_key;

        -- Grant to all cashier roles
        FOR v_role_record IN 
            SELECT id, name, tenant_id
            FROM roles
            WHERE LOWER(name) LIKE '%cashier%'
              AND space = 'tenant'
        LOOP
            INSERT INTO role_permissions (role_id, permission_id, created_at)
            VALUES (v_role_record.id, v_permission_id, NOW())
            ON CONFLICT DO NOTHING;
        END LOOP;
    END LOOP;
    
    RAISE NOTICE 'All cash permissions granted to cashier roles';
END $$;

-- Verify all cashier cash permissions
SELECT 
    r.name as role_name,
    r.tenant_id,
    p.permission_key
FROM roles r
JOIN role_permissions rp ON r.id = rp.role_id
JOIN permissions p ON rp.permission_id = p.id
WHERE LOWER(r.name) LIKE '%cashier%'
  AND (p.permission_key LIKE 'money-loan:cash:%' OR p.permission_key LIKE 'money-loan:collector%')
ORDER BY r.tenant_id, r.name, p.permission_key;
